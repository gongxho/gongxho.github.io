...
![在这里插入图片描述](https://img-blog.csdnimg.cn/7ce095f844204c06969fd55e644da2a1.png)
...
例如：`EIP（Extern Instruction Pointer ）寄存器`的值是下一条指令的地址，CPU执行完当前指令后，会根据 EIP 的值找到下一条指令，改变 EIP 的值，就会改变程序的执行流程；`CR3 寄存器`保存着当前进程页目录的物理地址，切换进程就会改变 CR3 的值，这将在《[MMU部件以及对内存权限的控制](#7MMU_268)》中讲解；EBP、ESP 寄存器用来指向栈的底部和顶部，函数调用会改变 EBP 和 ESP 的值，这将在《[栈的概念以及栈溢出](#11_418)》中讲解。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/25dfbda1b732462982d731ffb08e9c4f.png)
...
CPU指令
要想让CPU工作，必须借助特定的指令，例如 add 用于加法运算，sub 用于除法运算，cmp 用于比较两个数的大小，这称为`CPU的指令集（Instruction Set）`。我们的C语言代码最终也会编译成一条一条的CPU指令。不同型号的CPU支持的指令集会有所差异，但绝大部分是相同的。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/ac651c5569fc468d8509c329fe325742.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/3a24fce2fc014e188c1d674d4c7cbd8e.png)
### 虚拟地址
虚拟地址的整个想法是这样的：把程序给出的地址看做是一种`虚拟地址（Virtual Address）`，然后通过某些映射的方法，将这个虚拟地址转换成实际的物理地址。这样，只要我们能够妥善地控制这个虚拟地址到物理地址的映射过程，就可以保证程序每次运行时都可以使用相同的地址。
...
以上两点我们将在《[内存分页机制](#5_162)》和《[内存分页机制的实现](#6_215)》中进行详细讲解。
...
也就是说，对于32位的编译模式，不管实际物理内存有多大，程序能够访问的有效虚拟地址空间的范围就是`0 ~ 0XFFFFFFFF`，也即虚拟地址空间的大小是 4GB。换句话说，程序能够使用的最大内存为 4GB，跟物理内存没有关系。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/a7db420a4f434a7c9e2eb07dc922657b.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/b060ae17237b4c2780b219764e7516f6.png)
...
`以程序为单位对虚拟内存进行映射`时，如果物理内存不足，被换入换出到磁盘的是整个程序，这样势必会导致大量的磁盘读写操作，严重影响运行速度，所以这种方法还是显得粗糙，粒度比较大。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/fca5518f9c374b7683bdff999bd3b1af.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/a18ccd89d8fa45ca89c07c9555afe61e.png)


内存地址的转换是通过一种叫做`页表（Page Table）`的机制来完成的，这是本节要讲解的重点，即：
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/515f6b79244d413b8c80ab69f7675ff8.png)
...
例如，一个 int 类型的值保存在第 12 页，页内偏移为 240，那么对应的物理地址就是 `2^12 * 12 + 240` = 49392。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/7d0d21561633413ca9d851ea69b8b838.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/0cfd8fa204fc486e8ca8acca428b01ba.png)
...
### MMU
在CPU内部，有一个部件叫做`MMU（Memory Management Unit，内存管理单元）`，由它来负责将虚拟地址映射为物理地址，如下图所示：

![在这里插入图片描述](https://img-blog.csdnimg.cn/bd9a83400f1c44fe8a9fa0457adb2d14.png)
...
### 对内存权限的控制
MMU 除了能够完成虚拟地址到物理地址的映射，还能够对内存权限进行控制。上节《[内存分页机制的实现（虚拟地址和物理地址的映射）](#6_215)》讲到，在页表数组中，每个元素占用4个字节，也即32位，我们使用高20位来表示物理页编号，还剩下低12位，这12位就用来对内存进行控制，例如，是映射到物理内存还是映射到磁盘，程序有没有访问权限，当前页面有没有执行权限等。
...
在《[虚拟地址空间以及编译模式](#3_83)》一节中讲到，虚拟地址空间在32位环境下的大小为 4GB，在64位环境下的大小为 256TB，那么，一个C语言程序的内存在整个地址空间中是如何分布的呢？数据在哪里？代码在哪里？为什么要这样分布？这些就是本节要讲解的内容。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/00d361eb36e942ab8862f59c2a45d2f2.png)
对各个内存分区的说明：
...
在64位环境下，虚拟地址空间大小为 256TB，Linux 将高 128TB 的空间分配给内核使用，而将低 128TB 的空间分配给用户程序使用。如下图所示：
 ...
 ![在这里插入图片描述](https://img-blog.csdnimg.cn/ed4f9573c4874ad78e9443613ab3f9b3.png)



《[虚拟地址空间以及编译模式](#3_83)》一节中讲到，在64位环境下，虚拟地址虽然占用64位，但只有最低48位有效。这里需要补充的一点是，任何虚拟地址的48位至63位必须与47位一致。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/8ffae0ba664542e7aaa09864d62a05ea.png)
...
**用户程序调用系统 API 函数称为系统调用（System Call）**；发生系统调用时会暂停用户程序，转而执行内核代码（内核也是程序），访问内核空间，这称为`内核模式（Kernel Mode）`。
...
## 11、栈的概念以及栈溢出
在《[C语言程序的内存布局（内存模型）](#8LinuxC_293)》中我们讲到，程序的虚拟地址空间分为多个区域，栈（Stack）是其中地址较高的一个区域。
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/bfec145d19c4499b8feae8a53819e5ef.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/7982f028245d4c05bc9651a9321c83fc.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/7882be5fc1b64912b24363e7ba4706d4.png)
### 函数进栈
步骤①到⑥是函数进栈过程：
...
## 14、栈溢出攻击的原理
![在这里插入图片描述](https://img-blog.csdnimg.cn/4fe96ac05b9046018d052d210f8c89a5.png)
...
>注意：这里所说的“栈溢出”是指栈上的某个数据过大，覆盖了其他的数据，和《[栈的概念以及栈溢出](#11_418)》一节中提到的栈溢出不是一回事。
...
![图片描述](https://img-blog.csdnimg.cn/ee75e403b24c41c6af01e60b23799874.png "已分配内存和空闲内存相间出现")
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/1f0176f0d71f403faa3680a7ff0596a3.png "内存块的结构")

next 是指针，指向下一个内存块，used 用来表示当前内存块是否已被使用。这样，整个堆区就会形成如下图所示的链表：

![在这里插入图片描述](https://img-blog.csdnimg.cn/75cb88f7b6f04313ba9ea95f6a285348.png "类似链表的内存管理方式")

现在假设需要为程序分配100个字节的内存，当搜索到图中第一个空闲区域（大小为200个字节）时，发现满足条件，那么就在这里分配。这时候 malloc() 会把第一个空闲区域拆分成两部分，一部分交给程序使用，剩下的部分任然空闲，如下图所示：

![在这里插入图片描述](https://img-blog.csdnimg.cn/8803e861a2fd403ea7ee498f7cb3cde2.png "为程序分配100个字节的内存")

仍然以图3为例，当程序释放掉第三个内存块时，就会形成新的空闲区域，free() 会将第二、三、四个连续的空闲区域合并为一个，如下图所示：

![在这里插入图片描述](https://img-blog.csdnimg.cn/77f65fffd5c3497db7d302676a1355a8.png "释放第三个内存块")
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/e25f6524e63a48c4823c55b9bde5e4e6.png)
